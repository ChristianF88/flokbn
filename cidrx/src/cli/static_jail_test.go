...
// CIDRs and `cidrx static --jailFile X --banFile Y --clusterArgSets ...` wrote
...
func TestStaticFlagsModeJailsDetections(t *testing.T) {
	...
	output, err := runAppCaptured(t, []string{"cidrx", "static",
		"--logfile", logFile,
...
func TestLiveCommandRejectsStaticOnlyFlags(t *testing.T) {
	staticOnlyArgs := [][]string{
		{"cidrx", "live", "--plain"},
		{"cidrx", "live", "--compact"},
		{"cidrx", "live", "--rangesCidr", "10.0.0.0/8"},
		{"cidrx", "live", "--plotPath", "/tmp/plot.png"},
		{"cidrx", "live", "--tui"},
	}
