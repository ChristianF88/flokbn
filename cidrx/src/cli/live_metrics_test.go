...
func TestRenderMetrics_ValuesAndLabels(t *testing.T) {
	...
	wantLines := []string{
		"cidrx_uptime_seconds 42",
		"cidrx_ingest_connected 1",
		"cidrx_ingest_queue_depth 3",
		"cidrx_ingest_batches_total 7",
		"cidrx_ingest_requests_total 1234",
		"cidrx_ingest_parse_errors_total 2",
		"cidrx_ingest_malformed_fields_total 1",
		`cidrx_window_unique_ips{window="general_detection"} 11`,
		`cidrx_window_unique_ips{window="aggressive_detection"} 33`,
		`cidrx_window_requests{window="general_detection"} 200`,
		`cidrx_window_accepted_total{window="general_detection"} 500`,
		`cidrx_window_rejected_by_filter_total{window="general_detection"} 5`,
		`cidrx_cluster_detected{window="general_detection",set="50:24-32:0.20"} 1`,
		`cidrx_cluster_duration_seconds{window="general_detection",set="50:24-32:0.20"} 0.0015`,
		"cidrx_jail_active 4",
		`cidrx_ban_active{cidr="172.16.16.0/24",stage="1"} 1`,
		`cidrx_ban_active{cidr="172.16.1.0/27",stage="2"} 1`,
		`cidrx_jail_stage_active{stage="1"} 3`,
		`cidrx_jail_stage_active{stage="2"} 1`,
		"cidrx_ban_file_entries 6",
		`cidrx_ua_list_hits_total{list="whitelist"} 10`,
		`cidrx_ua_list_hits_total{list="blacklist"} 20`,
		`cidrx_ua_list_active_ips{list="whitelist"} 1`,
		`cidrx_ua_list_active_ips{list="blacklist"} 2`,
		"cidrx_loop_iterations_total 99",
		"cidrx_loop_duration_seconds 0.25",
	}
...
func TestRenderMetrics_LabelEscaping(t *testing.T) {
	...
	want := `cidrx_window_unique_ips{window="we\"ird\\name\nx"} 11`
	if !strings.Contains(got, want+"\n") {
...
func TestMetricsEndpoint(t *testing.T) {
	...
	if !strings.Contains(string(body), "cidrx_jail_active 4\n") {
